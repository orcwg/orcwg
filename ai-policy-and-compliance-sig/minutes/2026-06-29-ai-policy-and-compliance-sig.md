---
SIG: AI Policy and Compliance SIG
Document type: Minutes
Status: 📝 draft
Date: 2026-09-29
---

## Agenda
| Min | Agenda Topics | Moderator |
| --: | ----- | --- |
|   0 | Introduction of the AI Policy and Compliance SIG| Ciaran |
|   5 | | |
|  10 | Introduction of the AI Interest Group - coordination | Michael Berns |
|  15 | | |
|  20 | SIG Operations Guidelines | Sharon Corbett |
|  25 | | |
|  30 | AI Policy and Compliance SIG scope discussion | Dave Russo | 
|  35 | | |
|  40 | | |  
|  45 | | |
|  50 | Consultation on high-risk AI systems | Ciaran |
|  55 | AOB | |

## Participants

- Adrian O'Sullivan (Huawei)
- Alistair Woodman (Erlang Ecosystem Foundation)
- Bastien Guerry (Software Heritage)
- Ciarán O'Riordan (Eclipse Foundation)
- Dave Russo (Red Hat)
- Dirk-Willem van Gulik (Apache Software Foundation)
- Francisco Picolini (OpenNebula)
- Ian Liu (Open Culture Foundation)
- Isabel Drost-Fromm (Apache Software Foundation)
- Juan Rico (Eclipse Foundation)
- Kanchana Welagedara (Apache Software Foundation)
- Matt Albrecht (Zilliant)
- Marcel Kurzmann (Bosch)
- Marcello Martina (Eurotech)
- Marcus Schlichting (Krakun)
- Mathias Schindler (GitHub)
- Matteo Maiero (Eurotech)
- Michael Berns (Eclipse Foundation)
- Oliver Fendt (Siemens)
- Pavel Hruza (Red Hat)
- Rachel Foucard (TYPO3)
- Roman Zhukov (Red Hat)
- Salve J. Nilsen (CPANSec)
- Sharon Corbett (Eclipse Foundation)
- Timo Perälä (Nokia)

## Notes

- With holidays approaching, we agreed to not take a final decision before the summer on the scope of the SIG.  This will be discussed and presented for vote after the summer.
- Participants were asked to nominate themselves, by sending a mail to the list, if they wish to be a Co-Lead of this SIG.  We expect to vote on this in the following meeting in approximately two weeks' time.
- Sharon Corbet gave an overview of Eclipse organisation, SIGs, WGs, IGs
- Michael Berns presented the software projects the EF AI IG works on, including PanEval
- Overlap with other groups: yes, we'll try to minimise, but sometimes repeating a topic in this compliance group frees everyone from having to attend the meetings of a group which is mostly about another topic
- Representation and the purpose of collaboration: we hope to include many stakeholder categories so that we can find solutions that work for everyone 
- Possible output: the CRA FAQ was noted as being appreciated, could be replicated for AI Act.  Also whitepapers.
- We discussed the SIG's scope:
 - AI Act compliance: YES
 - AI Act ongoing implementation acts (standards, guidance, codes of practice...): YES
 - AI legislation in other countries: YES if there's interest
 - Tooling: YES, documenting and analysing, rather than developing tools (tools are being developed by EF, but projects are not owned by a WG / SIG / IG)
 - Security: YES, if there's a policy angle
 - Project policies, such as dealing with AI generated contributions: WE'LL SEE
- Coordination of interaction between CRA and AI Act: no decision, but we agreed to keep this in mind and keep both SIGs involved
- The group is invited to discuss the currently open EC consultation on high-risk AI
- An overview was given of other ongoing work (such as the AI Act's standards) and upcoming work (such as the EC consultation on the AI Act's research exemption)
